...
object StatisticsApp : AppDescriptor {
    ...
    override val controller: Controller = object : Controller {
        ...
        override fun process(node: Node) {
            runAsync {
                node to Statistics.fromNode(node)
            } ui {
                this .node = it.first
            ...
            init {
                with(root) {
                    form {
                        fieldset("位置信息") {
                            field("Deep") {
                                label(staP.stringBinding { it?.deep.toString() })
                            }
                            field("Children") {
                                label(staP.stringBinding { it?.childrenCount.toString() })
                            }
                            field("Descendant") {
                                label(staP.stringBinding { it?.descendantsCount.toString() })
                            }
                        }
